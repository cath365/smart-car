...
void writeServo(int pin, int angle, int minUs, int maxUs) {
...
void applyServos() {
  writeServo(SERVO_PAN,  curPan,  PAN_MIN_US,  PAN_MAX_US);
  writeServo(SERVO_TILT, curTilt, TILT_MIN_US, TILT_MAX_US);
}
...
void setup() {
  ...
  ledcAttach(SERVO_PAN,  PWM_FREQ, PWM_RES);
  ledcAttach(SERVO_TILT, PWM_FREQ, PWM_RES);
